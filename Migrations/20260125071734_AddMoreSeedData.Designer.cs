// <auto-generated />
using System;
using EduManager.InfraEstrutura.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace EduManager.Migrations
{
    [DbContext(typeof(EduManagerContext))]
    [Migration("20260125071734_AddMoreSeedData")]
    partial class AddMoreSeedData
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.AlunoDominio", b =>
                {
                    b.Property<int>("AlunoId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("AlunoId"));

                    b.Property<bool>("AlunoAtivo")
                        .HasColumnType("bit");

                    b.Property<string>("AlunoCPF")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("nvarchar(11)");

                    b.Property<DateTime>("AlunoDataMatricula")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("AlunoDataNascimento")
                        .HasColumnType("datetime2");

                    b.Property<string>("AlunoEmail")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("AlunoNomeCompleto")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int>("CursoId")
                        .HasColumnType("int");

                    b.Property<string>("UserId")
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("AlunoId");

                    b.HasIndex("AlunoCPF")
                        .IsUnique();

                    b.HasIndex("AlunoEmail")
                        .IsUnique();

                    b.HasIndex("CursoId");

                    b.HasIndex("UserId")
                        .IsUnique()
                        .HasFilter("[UserId] IS NOT NULL");

                    b.ToTable("Alunos", (string)null);

                    b.HasData(
                        new
                        {
                            AlunoId = 1,
                            AlunoAtivo = true,
                            AlunoCPF = "34567890123",
                            AlunoDataMatricula = new DateTime(2023, 8, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            AlunoDataNascimento = new DateTime(2000, 5, 15, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            AlunoEmail = "carlos@exemplo.com",
                            AlunoNomeCompleto = "Carlos Eduardo",
                            CursoId = 1
                        },
                        new
                        {
                            AlunoId = 2,
                            AlunoAtivo = true,
                            AlunoCPF = "45678901234",
                            AlunoDataMatricula = new DateTime(2023, 8, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            AlunoDataNascimento = new DateTime(1999, 10, 20, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            AlunoEmail = "ana@exemplo.com",
                            AlunoNomeCompleto = "Ana Paula",
                            CursoId = 2
                        },
                        new
                        {
                            AlunoId = 3,
                            AlunoAtivo = true,
                            AlunoCPF = "56789012345",
                            AlunoDataMatricula = new DateTime(2023, 8, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            AlunoDataNascimento = new DateTime(2001, 3, 10, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            AlunoEmail = "lucas@exemplo.com",
                            AlunoNomeCompleto = "Lucas Mendes",
                            CursoId = 1
                        });
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.ApplicationUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<bool>("Ativo")
                        .HasColumnType("bit");

                    b.Property<string>("CPF")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("nvarchar(11)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("DataCadastro")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("DataNascimento")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Sobrenome")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Telefone")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("CPF")
                        .IsUnique();

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("Usuarios", (string)null);
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.CoordenadorDominio", b =>
                {
                    b.Property<int>("CoordenadorId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("CoordenadorId"));

                    b.Property<string>("CoordenadorCPF")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("nvarchar(11)");

                    b.Property<DateTime>("CoordenadorDataContratacao")
                        .HasColumnType("datetime2");

                    b.Property<string>("CoordenadorEmail")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("CoordenadorNome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<decimal>("CoordenadorSalario")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("UserId")
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("CoordenadorId");

                    b.HasIndex("CoordenadorCPF")
                        .IsUnique();

                    b.HasIndex("CoordenadorEmail")
                        .IsUnique();

                    b.HasIndex("UserId")
                        .IsUnique()
                        .HasFilter("[UserId] IS NOT NULL");

                    b.ToTable("Coordenadores", (string)null);

                    b.HasData(
                        new
                        {
                            CoordenadorId = 1,
                            CoordenadorCPF = "67890123456",
                            CoordenadorDataContratacao = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CoordenadorEmail = "roberto@exemplo.com",
                            CoordenadorNome = "Prof. Roberto Lima",
                            CoordenadorSalario = 6000.00m
                        });
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.CursoDominio", b =>
                {
                    b.Property<int>("CursoId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("CursoId"));

                    b.Property<int>("CoordenadorId")
                        .HasColumnType("int");

                    b.Property<bool>("CursoAtivo")
                        .HasColumnType("bit");

                    b.Property<int>("CursoCargaHoraria")
                        .HasColumnType("int");

                    b.Property<DateTime>("CursoDataInicio")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("CursoDataTermino")
                        .HasColumnType("datetime2");

                    b.Property<string>("CursoDescricao")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("CursoNome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("CursoId");

                    b.HasIndex("CoordenadorId");

                    b.ToTable("Cursos", (string)null);

                    b.HasData(
                        new
                        {
                            CursoId = 1,
                            CoordenadorId = 1,
                            CursoAtivo = true,
                            CursoCargaHoraria = 0,
                            CursoDataInicio = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CursoDataTermino = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CursoDescricao = "Curso de Engenharia de Software",
                            CursoNome = "Engenharia de Software"
                        },
                        new
                        {
                            CursoId = 2,
                            CoordenadorId = 1,
                            CursoAtivo = true,
                            CursoCargaHoraria = 0,
                            CursoDataInicio = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CursoDataTermino = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CursoDescricao = "Curso de Ciência da Computação",
                            CursoNome = "Ciência da Computação"
                        });
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.DisciplinaDominio", b =>
                {
                    b.Property<int>("DisciplinaId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("DisciplinaId"));

                    b.Property<int>("CargaHoraria")
                        .HasColumnType("int");

                    b.Property<string>("Codigo")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<int>("CursoId")
                        .HasColumnType("int");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int>("ProfessorId")
                        .HasColumnType("int");

                    b.HasKey("DisciplinaId");

                    b.HasIndex("Codigo")
                        .IsUnique();

                    b.HasIndex("CursoId");

                    b.HasIndex("ProfessorId");

                    b.ToTable("Disciplinas", (string)null);

                    b.HasData(
                        new
                        {
                            DisciplinaId = 1,
                            CargaHoraria = 0,
                            Codigo = "POO101",
                            CursoId = 1,
                            Nome = "Programação Orientada a Objetos",
                            ProfessorId = 1
                        },
                        new
                        {
                            DisciplinaId = 2,
                            CargaHoraria = 0,
                            Codigo = "BD201",
                            CursoId = 1,
                            Nome = "Banco de Dados",
                            ProfessorId = 2
                        },
                        new
                        {
                            DisciplinaId = 3,
                            CargaHoraria = 0,
                            Codigo = "ALG101",
                            CursoId = 2,
                            Nome = "Algoritmos",
                            ProfessorId = 1
                        });
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.FrequenciaDominio", b =>
                {
                    b.Property<int>("FrequenciaId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("FrequenciaId"));

                    b.Property<int>("AlunoId")
                        .HasColumnType("int");

                    b.Property<DateTime>("DataAula")
                        .HasColumnType("datetime2");

                    b.Property<int>("DisciplinaId")
                        .HasColumnType("int");

                    b.Property<bool>("Presente")
                        .HasColumnType("bit");

                    b.Property<int>("TurmaId")
                        .HasColumnType("int");

                    b.HasKey("FrequenciaId");

                    b.HasIndex("AlunoId");

                    b.HasIndex("DisciplinaId");

                    b.HasIndex("TurmaId");

                    b.ToTable("Frequencias", (string)null);

                    b.HasData(
                        new
                        {
                            FrequenciaId = 1,
                            AlunoId = 1,
                            DataAula = new DateTime(2023, 9, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DisciplinaId = 1,
                            Presente = true,
                            TurmaId = 1
                        },
                        new
                        {
                            FrequenciaId = 2,
                            AlunoId = 2,
                            DataAula = new DateTime(2023, 9, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DisciplinaId = 2,
                            Presente = false,
                            TurmaId = 2
                        },
                        new
                        {
                            FrequenciaId = 3,
                            AlunoId = 3,
                            DataAula = new DateTime(2023, 9, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DisciplinaId = 1,
                            Presente = true,
                            TurmaId = 1
                        });
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.NotaDominio", b =>
                {
                    b.Property<int>("NotaId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("NotaId"));

                    b.Property<int>("AlunoId")
                        .HasColumnType("int");

                    b.Property<DateTime>("DataAvaliacao")
                        .HasColumnType("datetime2");

                    b.Property<int>("DisciplinaId")
                        .HasColumnType("int");

                    b.Property<string>("TipoAvaliacao")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<decimal>("Valor")
                        .HasPrecision(5, 2)
                        .HasColumnType("decimal(5,2)");

                    b.HasKey("NotaId");

                    b.HasIndex("AlunoId");

                    b.HasIndex("DisciplinaId");

                    b.ToTable("Notas", (string)null);

                    b.HasData(
                        new
                        {
                            NotaId = 1,
                            AlunoId = 1,
                            DataAvaliacao = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DisciplinaId = 1,
                            TipoAvaliacao = "Prova 1",
                            Valor = 8.5m
                        },
                        new
                        {
                            NotaId = 2,
                            AlunoId = 2,
                            DataAvaliacao = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DisciplinaId = 2,
                            TipoAvaliacao = "Prova 1",
                            Valor = 9.0m
                        },
                        new
                        {
                            NotaId = 3,
                            AlunoId = 3,
                            DataAvaliacao = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DisciplinaId = 1,
                            TipoAvaliacao = "Prova 1",
                            Valor = 7.5m
                        });
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.ProfessorDominio", b =>
                {
                    b.Property<int>("ProfessorId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ProfessorId"));

                    b.Property<string>("CPF")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("nvarchar(11)");

                    b.Property<DateTime>("DataContratacao")
                        .HasColumnType("datetime2");

                    b.Property<string>("Especialidade")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("ProfessorNome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<decimal>("Salario")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("UserId")
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("ProfessorId");

                    b.HasIndex("CPF")
                        .IsUnique();

                    b.HasIndex("UserId")
                        .IsUnique()
                        .HasFilter("[UserId] IS NOT NULL");

                    b.ToTable("Professores", (string)null);

                    b.HasData(
                        new
                        {
                            ProfessorId = 1,
                            CPF = "12345678901",
                            DataContratacao = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Especialidade = "Programação",
                            ProfessorNome = "Dr. João Pereira",
                            Salario = 5000.00m
                        },
                        new
                        {
                            ProfessorId = 2,
                            CPF = "23456789012",
                            DataContratacao = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Especialidade = "Banco de Dados",
                            ProfessorNome = "Dra. Maria Silva",
                            Salario = 5500.00m
                        });
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.TurmaDominio", b =>
                {
                    b.Property<int>("TurmaId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("TurmaId"));

                    b.Property<string>("Ano")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<bool>("Ativa")
                        .HasColumnType("bit");

                    b.Property<int>("CursoId")
                        .HasColumnType("int");

                    b.Property<DateTime>("DataFim")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("DataInicio")
                        .HasColumnType("datetime2");

                    b.Property<int>("DisciplinaId")
                        .HasColumnType("int");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("ProfessorId")
                        .HasColumnType("int");

                    b.Property<int>("Semestre")
                        .HasColumnType("int");

                    b.HasKey("TurmaId");

                    b.HasIndex("CursoId");

                    b.HasIndex("DisciplinaId");

                    b.HasIndex("ProfessorId");

                    b.ToTable("Turmas", (string)null);

                    b.HasData(
                        new
                        {
                            TurmaId = 1,
                            Ano = "2023",
                            Ativa = true,
                            CursoId = 1,
                            DataFim = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DataInicio = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DisciplinaId = 1,
                            Nome = "Turma A - POO",
                            ProfessorId = 1,
                            Semestre = 0
                        },
                        new
                        {
                            TurmaId = 2,
                            Ano = "2023",
                            Ativa = true,
                            CursoId = 1,
                            DataFim = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DataInicio = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DisciplinaId = 2,
                            Nome = "Turma B - BD",
                            ProfessorId = 2,
                            Semestre = 0
                        },
                        new
                        {
                            TurmaId = 3,
                            Ano = "2023",
                            Ativa = true,
                            CursoId = 2,
                            DataFim = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DataInicio = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DisciplinaId = 3,
                            Nome = "Turma A - Alg",
                            ProfessorId = 1,
                            Semestre = 0
                        });
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("RoleId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens", (string)null);
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.AlunoDominio", b =>
                {
                    b.HasOne("EduManager.Models.Entities.Dominios.CursoDominio", "Curso")
                        .WithMany("Alunos")
                        .HasForeignKey("CursoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("EduManager.Models.Entities.Dominios.ApplicationUser", "User")
                        .WithOne("Aluno")
                        .HasForeignKey("EduManager.Models.Entities.Dominios.AlunoDominio", "UserId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Curso");

                    b.Navigation("User");
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.CoordenadorDominio", b =>
                {
                    b.HasOne("EduManager.Models.Entities.Dominios.ApplicationUser", "User")
                        .WithOne("Coordenador")
                        .HasForeignKey("EduManager.Models.Entities.Dominios.CoordenadorDominio", "UserId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("User");
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.CursoDominio", b =>
                {
                    b.HasOne("EduManager.Models.Entities.Dominios.CoordenadorDominio", "Coordenador")
                        .WithMany("Cursos")
                        .HasForeignKey("CoordenadorId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Coordenador");
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.DisciplinaDominio", b =>
                {
                    b.HasOne("EduManager.Models.Entities.Dominios.CursoDominio", "Curso")
                        .WithMany("Disciplinas")
                        .HasForeignKey("CursoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("EduManager.Models.Entities.Dominios.ProfessorDominio", "Professor")
                        .WithMany("Disciplinas")
                        .HasForeignKey("ProfessorId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Curso");

                    b.Navigation("Professor");
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.FrequenciaDominio", b =>
                {
                    b.HasOne("EduManager.Models.Entities.Dominios.AlunoDominio", "Aluno")
                        .WithMany("Frequencias")
                        .HasForeignKey("AlunoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("EduManager.Models.Entities.Dominios.DisciplinaDominio", "Disciplina")
                        .WithMany("Frequencias")
                        .HasForeignKey("DisciplinaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("EduManager.Models.Entities.Dominios.TurmaDominio", "Turma")
                        .WithMany("Frequencias")
                        .HasForeignKey("TurmaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Aluno");

                    b.Navigation("Disciplina");

                    b.Navigation("Turma");
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.NotaDominio", b =>
                {
                    b.HasOne("EduManager.Models.Entities.Dominios.AlunoDominio", "Aluno")
                        .WithMany("Notas")
                        .HasForeignKey("AlunoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("EduManager.Models.Entities.Dominios.DisciplinaDominio", "Disciplina")
                        .WithMany("Notas")
                        .HasForeignKey("DisciplinaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Aluno");

                    b.Navigation("Disciplina");
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.ProfessorDominio", b =>
                {
                    b.HasOne("EduManager.Models.Entities.Dominios.ApplicationUser", "User")
                        .WithOne("Professor")
                        .HasForeignKey("EduManager.Models.Entities.Dominios.ProfessorDominio", "UserId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("User");
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.TurmaDominio", b =>
                {
                    b.HasOne("EduManager.Models.Entities.Dominios.CursoDominio", "Curso")
                        .WithMany()
                        .HasForeignKey("CursoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("EduManager.Models.Entities.Dominios.DisciplinaDominio", "Disciplina")
                        .WithMany("Turmas")
                        .HasForeignKey("DisciplinaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("EduManager.Models.Entities.Dominios.ProfessorDominio", "Professor")
                        .WithMany("Turmas")
                        .HasForeignKey("ProfessorId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Curso");

                    b.Navigation("Disciplina");

                    b.Navigation("Professor");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("EduManager.Models.Entities.Dominios.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("EduManager.Models.Entities.Dominios.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("EduManager.Models.Entities.Dominios.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("EduManager.Models.Entities.Dominios.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.AlunoDominio", b =>
                {
                    b.Navigation("Frequencias");

                    b.Navigation("Notas");
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.ApplicationUser", b =>
                {
                    b.Navigation("Aluno");

                    b.Navigation("Coordenador");

                    b.Navigation("Professor");
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.CoordenadorDominio", b =>
                {
                    b.Navigation("Cursos");
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.CursoDominio", b =>
                {
                    b.Navigation("Alunos");

                    b.Navigation("Disciplinas");
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.DisciplinaDominio", b =>
                {
                    b.Navigation("Frequencias");

                    b.Navigation("Notas");

                    b.Navigation("Turmas");
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.ProfessorDominio", b =>
                {
                    b.Navigation("Disciplinas");

                    b.Navigation("Turmas");
                });

            modelBuilder.Entity("EduManager.Models.Entities.Dominios.TurmaDominio", b =>
                {
                    b.Navigation("Frequencias");
                });
#pragma warning restore 612, 618
        }
    }
}
